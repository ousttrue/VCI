...
namespace UniGLTF.Legacy
{
    ...
    public class LegacyImporterContext : IDisposable
    {
                    ...
                    for (int i = 0; i < GLTF.meshes.Count; ++i)
                    {
                        ...
                        parent.AddTask(Scheduler.ThreadPool,
                                () =>
                                {
                                    ...
                                    {
                                        return meshImporter.ReadMesh(new GltfParser { GLTF = GLTF, Storage = Storage }, index, Axises.Z.Create());
                                    }
                                })
                ...
                .ContinueWith(Scheduler.MainThread, _ =>
                {
                    using (MeasureTime("AnimationImporter"))
                    {
                        if (GLTF.animations != null && GLTF.animations.Any())
                        {
                            ...
                            AnimationClips = AnimationImporter.ImportAnimationClips(GLTF, Axises.Z).ToList();
        ...
        IEnumerator LoadMeshes()
        {
            ...
            for (int i = 0; i < GLTF.meshes.Count; ++i)
            {
                var meshContext = meshImporter.ReadMesh(new GltfParser{GLTF=GLTF, Storage=Storage}, i, Axises.Z.Create());
